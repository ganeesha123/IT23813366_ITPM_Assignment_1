    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if input_by_ph.count() > 0 and output_by_ph.count() > 0 and input_by_ph.is_visible() and output_by_ph.is_visible():
                # Try multiple button name patterns
                action = None
                for btn_pattern in [r"Transliterate", r"Convert", r"Translate", r"Submit", r"Go", r"Send"]:
                    try:
                        btn = page.get_by_role("button", name=re.compile(btn_pattern, re.IGNORECASE)).first
